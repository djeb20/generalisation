...
class DQN:
    ...
    def __init__(self, state_dim, action_dim,
                 critic_arch=[40, 30], buffer_size=10000, batch_size=64,
    ...
    def make_critic(self, state_dim, action_dim, critic_arch):
        ...
        #     # This version takes in a state and action as input then outputs a single value.

        #     # Two inputs, state and action
        #     inp_state = Input(shape=(state_dim,))
        #     inp_action = Input(shape=(action_dim,))

        #     out = Concatenate()([inp_state, inp_action])
        #     for size in critic_arch:
        #         out = Dense(size, activation='relu')(out)

        #     # Output layer for value
        #     out = Dense(1)(out)

        #     return keras.Model([inp_state, inp_action], out)
    ...
    def update_critic(self, old_states, actions, rewards, new_states, terms):
        ...
        #         pos_actions_ind = tf.range(tf.shape(self.pos_actions)[0])
        #         mesh = tf.reshape(tf.transpose(tf.meshgrid(buf_ind, pos_actions_ind)), (-1, 2))

        #         input_states = tf.gather(new_states, mesh[:, 0])
        #         input_actions = tf.gather(self.pos_actions, mesh[:, 1])

        #         Q_max = tf.math.reduce_max(tf.reshape(self.target_critic([input_states, input_actions], training=True), 
        #         (b_size, tf.shape(self.pos_actions)[0])), axis=1)
        #         Q_values_actions = self.critic([old_states, actions])
